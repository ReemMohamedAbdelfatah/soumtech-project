...
import { Auction } from "@/lib/mocks/auctions";
import { TabType } from "../shared/AuctionTabFilter";
...
export default function AuctionGrid({
  ...
  const filteredAuctions = auctions.filter(
    (auction) => auction.status === filterTabValue,
  );
  return (
    <ul>
      {filteredAuctions.map((auction) => (
        <li key={auction.id}>
          <h1 className="ds-text-accent">{auction.title}</h1>
